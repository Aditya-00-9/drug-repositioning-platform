...
export default function HeaderSearch() {
  ...
  // Enter: go to profile if single match (approved list only)
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (!q) return;
    const exactDrug =
      drugsFromSheet.find((d) => d.id.toLowerCase() === q) ||
      drugsFromSheet.find((d) => d.name.toLowerCase() === q);
    if (exactDrug) {
      setShowDropdown(false);
    ...
    const exactDisease =
      searchDiseases(query).find((d) => d.id.toLowerCase() === q) ||
      searchDiseases(query).find((d) => d.name.toLowerCase() === q);
    if (exactDisease) {
      setShowDropdown(false);
    ...
    <div ref={wrapperRef} className="relative flex items-center gap-2">
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search approved drugs, diseases…"
