...
function TargetProfilePageClient({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const target: TargetProfile | undefined = getTargetById(resolvedParams.id);
      ...
      <main className="min-h-screen bg-gradient-to-b from-navy-900 via-black to-black">
        <div className="mx-auto max-w-6xl px-6 py-24">
          <div className="rounded-2xl border border-red-500/30 bg-red-950/20 p-8 text-center">
            <p className="text-lg font-medium text-red-400">Target not found</p>
            ...
            >
              <ArrowLeft size={16} /> Back to target list
            </Link>
          </div>
        </div>
      </main>
  ...
  const byTier = (tier: EvidenceTier): EvidenceRecord[] =>
    target.evidence.filter((e) => e.tier === tier);
  ...
  const totalEvidence = target.evidence.length;
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-navy-900 via-black to-black">
      ...
      <div className="relative mx-auto max-w-6xl px-6 pb-24 pt-8">
        ...
        >
          <ArrowLeft size={14} /> Back to target list
        </Link>
        ...
        <div className="mb-8 flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
          <div className="flex-1">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                <h1 className="font-mono text-3xl font-semibold tracking-tight text-slate-50 md:text-4xl">
                  {target.name}
                </h1>
                <p className="mt-1 text-xs font-mono uppercase tracking-[0.2em] text-slate-500">
                  Target-centric evidence profile
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="mb-12 rounded-2xl border border-slate-800/80 bg-slate-950/60 p-8 shadow-[0_26px_90px_rgba(15,23,42,0.9)]">
          <div className="mb-6 flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-slate-50">Target Snapshot</h2>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-base font-medium text-slate-200">{target.type}</p>
              </div>
              <div>
                <p className="mb-1 text-xs font-mono uppercase tracking-[0.2em] text-slate-500">
                  Pathway
                </p>
                <p className="text-sm leading-relaxed text-slate-300">{target.pathway}</p>
              </div>
            </div>
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-slate-300">{target.ligandSpace}</p>
              </div>
              <div>
                <p className="mb-1 text-xs font-mono uppercase tracking-[0.2em] text-slate-500">
          ...
          <div className="mt-6">
            <p className="mb-3 text-sm font-medium text-slate-300">Linked compounds</p>
            <div className="flex flex-wrap gap-2">
              {target.linkedDrugs.map((d) => (
                <Link
                  key={d.drugId}
                  href={`/drug/${d.drugId}`}
...
export default function TargetProfilePage({ params }: { params: Promise<{ id: string }> }) {
  return <TargetProfilePageClient params={params} />;
}
